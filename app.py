...
def send_reset_email(to_email: str, reset_token: str):
    ...
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#04080f;color:#eef2ff;padding:40px;border-radius:12px;">
      ...
      <p style="color:#8fa3c4;line-height:1.7;margin-bottom:32px;">
        We received a request to reset the password for your Claimflowpay account. 
      ...
      <p style="color:#5a7299;font-size:12px;">Claimflowpay · HIPAA Compliant · claimflowpay.us</p>
    </div>
    """
    try:
        response = httpx.post(
            ...
            json={"from": "Claimflowpay <noreply@claimflowpay.com>", "to": [to_email],
                  "subject": "Reset your Claimflowpay password", "html": html},
            timeout=10
        )
...
def serve_login():
    ...
    return FileResponse(p, media_type="text/html") if os.path.exists(p) else {"message": "Claimflowpay API running"}
...
@app.get("/health")
def health():
    return {"message": "Claimflowpay API is running!"}
...
def send_confirmation_email(email: str, practice_name: str, confirm_token: str):
    ...
    <body style="background:#0a0a0a;font-family:'DM Mono',monospace,sans-serif;padding:40px 24px;color:#e0e0e0">
      <div style="max-width:520px;margin:0 auto">
        ...
        <p style="font-size:0.85rem;color:#888;line-height:1.7;margin-bottom:32px">
          ...
          Thanks for signing up for Claimflowpay. Click the button below to confirm your email and activate your account.
        </p>
        ...
        <p style="font-size:0.72rem;color:#444;margin-top:32px;line-height:1.6">
          If you didn't create a Claimflowpay account, you can safely ignore this email.<br/>
    ...
    try:
        ...
        _httpx.post(
            "https://api.resend.com/emails",
            headers={"Authorization": f"Bearer {RESEND_API_KEY}", "Content-Type": "application/json"},
            json={"from": "Claimflowpay <noreply@claimflowpay.com>", "to": [email],
                  "subject": "Confirm your Claimflowpay account", "html": html},
            timeout=10
        )
    except Exception:
...
def claim_summary_pdf(claim_id: int, user=Depends(verify_token)):
    ...
    story.append(Paragraph(
        f"Generated by Claimflowpay  ·  {datetime.utcnow().strftime('%B %d, %Y  %H:%M UTC')}  ·  HIPAA Compliant  ·  claimflowpay.com",
        foot_style
    ))
    story.append(Paragraph(
